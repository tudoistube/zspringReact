...
public class BoardController {
	...
	//
	// /*
	// * ...196p. Model클래스는 스프링MVC에서 제공하는 데이터 전달용 객체이고, Map처럼 ...(key, value)로
	// * 구성되어 데이터를 저장함. ...Servlet에서는 RequestDispatcher에 데이터를 저장했음.
	// */
	...
	// */
	// //return "/yboard/success";// ...207p.주석처리.
	//
	// /*
	...
	// */
	// return "redirect:/yboard/listAll";
	//
	// }
	...
	/*
	 * ...207p. ...214p.listAll(Model m)은 MVC의 Model타입의 객체를 파라미터로 받아서, ...모든
	 * 게시물을 model.addAttribute()로 JSP에 전달함.
	 */
	 ...
	 * ...Model.addAttribute()작업할때 아무런 이름 없이 데이터를 넣으면 자동으로 ...클래스의 이름을 소문자로 시작해서
	 * 사용한다. ...BoardVO org.joywins.service.IF_BoardService.read(Integer bno)
	...
	/*
	 * ...225p.@RequestParam("bno")를 제외해도 bno에 값이 들어옴. ...addFlashAttribute()를
	 * 사용해서 한번만 실행되어서 새로고침시 계속 데이터가 전송되지 않게 함.
	 */
	...
	public String delete(@RequestParam("bno") int bno, 
		...
		return "redirect:/yboard/listPage";
	}
	...
	public void modifyGET(int bno, Model model) throws Exception {
		logger.info("수정페이지로 이동, update called get.../ bno = " + bno);
		model.addAttribute(service.read(bno));
	}

...
<script>				
	$(document).ready(function(){
		...
		$("#modifyBtn").on("click", function(){
			...
			//...수정화면으로 이동(get).
			formObj.attr("action", "/y2/ysboard/updatePage");
		...
		$("#deleteBtn").on("click", function(){
			...
			formObj.attr("action", "/y2/ysboard/deletePage");
			formObj.submit();
		});
		...
		$("#listBtn").on("click", function(){
			...
			//...294p.내용조회후 다시 원래 목록 위치로 돌아가기 위한 정보추가.
			formObj.attr("method", "get");
			formObj.attr("action", "/y2/ysboard/listPage");
			formObj.submit();		
		});
...
<script>
	...
	//...437p.최초로 댓글의 1페이지를 가져오는 작업은 화면상에서 'Replies List'라는 버튼을 
	//   클릭했을때 댓글 목록을 가져와서 보임.
	//...★ /y2/replies/로 요청경로를 한 이유를 알아야 함.
	...
	$(".pagination").on("click", "li a", function(event){
		...
		getPage("/y2/replies/"+bno+"/"+replyPage);

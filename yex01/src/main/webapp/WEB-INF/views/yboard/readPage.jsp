...
<%-- 
	...
	...수정하고 전송하게 함.
	...Tomcat에서 프로젝트실행경로를 199p.처럼 /y/로 한 경우, 페이지에서는 http://localhost:8080
	...다음에 오는 완전한 경로를 명시해서 보내야 처리가 됨.
	...ex)http://localhost:8080/y/yboard/read?bno=3
	...컨트롤러입장에서는 http://localhost:8080/y 다음에 오는 완전한 요청경로를 대상으로 처리함.	
 --%>
<script>				
$(document).ready(function(){
	...
	$(".btn-warning").on("click", function(){
		//formObj.attr("action", "/y/yboard/update"); //...read.jsp, 298p.주석.
		//...수정화면으로 이동(get).
		formObj.attr("action", "/y/yboard/updatePage");
	...
	$(".btn-danger").on("click", function(){
		...
		formObj.attr("action", "/y/yboard/deletePage");
		formObj.submit();
	});
	...
	$(".btn-primary").on("click", function(){
		...
		//...294p.내용조회후 다시 원래 목록 위치로 돌아가기 위한 정보추가.
		formObj.attr("method", "get");
		formObj.attr("action", "/y/yboard/listPage");
		formObj.submit();	
	});
